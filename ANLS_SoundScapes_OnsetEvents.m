...
% Reject epochs with multiple events?
% Set 'true' to reject epochs that have multiple events
epoch_clean = true; 
...
for iFile=1:length(audiofiles)
    ...
    if epoch_clean 
        for e = 1:length(EEG.epoch)
            if length(EEG.epoch(e).event) == 1
                epochidx(end+1) = e; 
            end
        end
        epochidx_all = [epochidx_all, epochidx + nepochs];

    else 
        epochidx = 1:length(EEG.epoch);
        epochidx_all = [epochidx_all, (1:length(EEG.epoch)) + nepochs];
    end 
    nepochs = nepochs + length(EEG.epoch);
    ...
    %%subplot(2,2, iFile)
    plot(EEG.times, mean(mean(EEG.data(channels2plot,:,epochidx) ,3),1), clr(iFile), 'LineWidth', s.plt_linewidth)
...
% plotting all soundscape ERP
%%subplot(2,2, iFile+1)
tasknames{end+1} = 'average'; 
plot(EEG.times, mean(mean(eegdata(channels2plot,:,epochidx_all) ,3),1), clr(iFile+1), 'LineWidth', s.plt_linewidth+.7)
